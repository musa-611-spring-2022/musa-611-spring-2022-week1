...
ADDED_MODIFIED_FILES = os.environ.get('ADDED_MODIFIED_FILES', '')
...
def test_start_file_has_not_been_modified():
    ...
    assert 'introductions/_start.html' not in ADDED_MODIFIED_FILES, \
           MODIFIED_START_FILE_MESSAGE
...
def test_new_html_file_has_been_added():
    ...
    assert html_file_pattern.search(ADDED_MODIFIED_FILES) is not None, \
           NO_NEW_HTML_FILES_MESSAGE
...
def test_new_html_file_not_called_yourname():
    ...
    yourname_pattern = re.compile(r'introductions/(yourname)\.html',
                                  re.IGNORECASE)
    youname_file_name = yourname_pattern.search(ADDED_MODIFIED_FILES)
